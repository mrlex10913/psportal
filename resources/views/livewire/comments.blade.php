    ...
    <div class="w-6/12">
      ...
      <div>
        @if (session()->has('message'))
            <div class="p-3 bg-green-300 text-green-700 rounded-sm shadow-md">
                {{session('message')}}
            </div>
        @endif
        @if (session()->has('delete'))
        <div class="p-3 bg-[#ffbb55] text-[#fff] rounded-sm shadow-md">
            {{session('delete')}}
        </div>
    @endif
      </div>
      ...
      <section>
        ...
        <form class="my-4 flex" wire:submit.prevent="addComment">
          <input type="text" class="w-full rounded border shadow p-2 mr-2 my-2" placeholder="What's in your mind." wire:model.debaounce.500ms="newComment">
          <div class="py-2">
